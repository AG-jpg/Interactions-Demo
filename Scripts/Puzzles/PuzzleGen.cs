...
public class PuzzleGen : MonoBehaviour
{
    ...
    {
        Vector3 camPos = puzzleCam.transform.position;
        camPos.x = _level.Col * 0.5f;
        camPos.y = _level.Row * 0.5f;
        puzzleCam.transform.position = camPos;
        puzzleCam.orthographicSize = Mathf.Max(_level.Row, _level.Col) + 2f;
    ...
    private void Update()
    {
            ...
            {
                Vector3 mousePos = puzzleCam.ScreenToWorldPoint(Input.mousePosition);
